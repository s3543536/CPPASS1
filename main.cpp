...
int main(int argc, char **argv) {
	...
	if(data_str == "list") {//std::list
		...
		//load the files
		std::vector<std::string> dict = wrap.load_dict(var_map["dictionary"].as<std::string>());
		if(dict.size() < 1) {
			std::cout << "Can't open Dictionary File\n";
			return EXIT_SUCCESS;
		}
		std::vector<std::string> text = wrap.load_text(var_map["text"].as<std::string>());
		if(text.size() < 1) {
			std::cout << "Can't open Text File\n";
			return EXIT_SUCCESS;
		}
		...
		//load the files
		std::map<std::string, int> dict = wrap.load_dict(var_map["dictionary"].as<std::string>());
		if(dict.size() < 1) {
			std::cout << "Can't open Dictionary File\n";
			return EXIT_SUCCESS;
		}
		std::map<std::string, int> text = wrap.load_text(var_map["text"].as<std::string>());
		if(text.size() < 1) {
			std::cout << "Can't open Text File\n";
			return EXIT_SUCCESS;
		}
